...
module.exports = {
  'User Registration Test': function (browser) {
    ...
    browser.assert.containsText('header.shepherd-header>h3', 'NEW: Technical Publications Experience')

    // checking paragraph in popup
    browser.assert.textContains('.shepherd-text', 'Easily find and download schematics and manuals (Parts, Operation, Service and Supplement) all in one place.')

    // checking for done button
    browser.assert.containsText('body>div.shepherd-has-cancel-icon.shepherd-has-title.shepherd-element.shepherd-centered.shepherd-enabled>div>footer>button', 'DONE')
    ...
    browser.assert.containsText('h3.ng-star-inserted', 'TERMS OF USE')
    //   // checking for presence of click to read terms of use
    browser.assert.containsText('a[href="https://www.jlg.com/en/terms"]', 'Click to read Terms of Use')
    ...
    browser.windowHandle(function (result) {
      originalHandle = result.value;
    });
    // clicking on click to read terms of use
    browser.click('a[href="https://www.jlg.com/en/terms"]');
    ...
    browser.windowHandles(function (result) {
      ...
      browser.switchWindow(originalHandle);
    });
    ...
    // switching into different window tab
    browser.click('a[href="https://www.jlg.com/en/privacy-policy"]');
    // waiting for body for complete load 
    browser.waitForElementVisible('body')
    browser.pause(7000);
    ...
    browser.assert.containsText('mat-vertical-stepper[_ngcontent-ole-app-c167]>div:nth-of-type(2)>mat-step-header>div:nth-of-type(2)>div>span', '2')
    // checking for presence of confirm user type
    browser.assert.containsText('mat-vertical-stepper[_ngcontent-ole-app-c167]>div:nth-of-type(2)>mat-step-header>div:nth-of-type(3)>div', 'Confirm User Type')
    // checking for presence of paragraph
    browser.assert.containsText('#cdk-step-content-0-1>div>div>div>p:nth-child(1):nth-child(1)', 'Does your company have a JLG Account Number?')
    // checking for presence of help icon
    browser.assert.containsText('#cdk-step-content-0-1>div>div>div>p:nth-child(1)>mat-icon', 'help')
